 ...
 */
class SagaBuilder<C : SagaContext>(private val sagaName: String) {
    private val steps = mutableListOf<SagaStep<C>>()
    private var retryPolicy = RetryPolicy.default()
    private var onStepComplete: (suspend (String, StepStatus) -> Unit)? = null
    private var onStepFailed: (suspend (String, Throwable) -> Unit)? = null
    private var onSagaComplete: (suspend (java.util.UUID, SagaState) -> Unit)? = null

    fun step(step: SagaStep<C>) = apply { steps.add(step) }

    fun retryPolicy(policy: RetryPolicy) = apply { retryPolicy = policy }

    fun onStepComplete(callback: suspend (String, StepStatus) -> Unit) = apply {
        onStepComplete = callback
    }

    fun onStepFailed(callback: suspend (String, Throwable) -> Unit) = apply {
        onStepFailed = callback
    }

    fun onSagaComplete(callback: suspend (java.util.UUID, SagaState) -> Unit) = apply {
        onSagaComplete = callback
    }

    fun build(): SagaOrchestrator<C> = SagaOrchestrator(
        sagaName = sagaName,
        steps = steps.toList(),
        retryPolicy = retryPolicy,
        onStepComplete = onStepComplete,
        onStepFailed = onStepFailed,
        onSagaComplete = onSagaComplete
    )
}

/**
 * DSL function to create a saga.
 */
fun <C : SagaContext> saga(name: String, block: SagaBuilder<C>.() -> Unit): SagaOrchestrator<C> {
    return SagaBuilder<C>(name).apply(block).build()
}
