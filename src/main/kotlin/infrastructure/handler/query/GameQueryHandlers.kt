...
/**
 * Query handler for finding a game by ID.
 */
class FindGameByIdQueryHandler : QueryHandler<FindGameByIdQuery, GameDetailsReadModel?> {
    override suspend fun handle(query: FindGameByIdQuery): GameDetailsReadModel? = newSuspendedTransaction {
        buildFullGameQuery()
            .andWhere { GameTable.id eq query.id }
            .singleOrNull()
...
/**
 * Query handler for finding a game by identity.
 */
class FindGameByIdentityQueryHandler : QueryHandler<FindGameByIdentityQuery, GameDetailsReadModel?> {
    override suspend fun handle(query: FindGameByIdentityQuery): GameDetailsReadModel? = newSuspendedTransaction {
        buildFullGameQuery()
            .andWhere { GameTable.identity eq query.identity }
            .singleOrNull()
            ?.let { GameDetailsReadModel.from(it.toGameWithDetails()) }
    }
}
...
/**
 * Query handler for finding a game by symbol.
 */
class FindGameBySymbolQueryHandler : QueryHandler<FindGameBySymbolQuery, GameDetailsReadModel?> {
    override suspend fun handle(query: FindGameBySymbolQuery): GameDetailsReadModel? = newSuspendedTransaction {
        buildFullGameQuery()
            .andWhere { GameVariantTable.symbol eq query.symbol }
            .singleOrNull()
            ?.let { GameDetailsReadModel.from(it.toGameWithDetails()) }
    }
}
...
/**
 * Query handler for listing games with pagination and filtering.
 * Uses optimized 3-query pattern to avoid N+1 and cartesian explosion.
 */
class ListGamesQueryHandler : QueryHandler<ListGamesQuery, Page<GameListReadModel>> {
    override suspend fun handle(query: ListGamesQuery): Page<GameListReadModel> = newSuspendedTransaction {
...
/**
 * Query handler for searching games.
 */
class SearchGamesQueryHandler : QueryHandler<SearchGamesQuery, List<GameSummaryReadModel>> {
    override suspend fun handle(query: SearchGamesQuery): List<GameSummaryReadModel> = newSuspendedTransaction {
        GameTable
            ...
            .where {
                GameTable.name.ilike(query.query) or GameTable.identity.ilike(query.query)
            }
