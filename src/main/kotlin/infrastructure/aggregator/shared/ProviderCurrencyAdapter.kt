...
class ProviderCurrencyAdapter(private val currencyAdapter: CurrencyAdapter) {
    suspend fun convertSystemToProvider(amount: Long, currency: Currency): BigDecimal {
        return currencyAdapter.convertFromSystem(amount, currency)
    }

    suspend fun convertProviderToSystem(amount: BigDecimal, currency: Currency): Long {
